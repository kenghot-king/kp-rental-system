...
class ProductTemplate(models.Model):
    ...
    qty_in_rent = fields.Float(
        "Quantity currently in rent",
        compute='_get_qty_in_rent',
    )
    ...
    extra_daily = fields.Float(
        "Daily Fine", help="Fine per day late", company_dependent=True,
    )
    ...
    def _get_qty_in_rent(self):
        ...
        for template in rentable:
            template.qty_in_rent = sum(
                template.mapped('product_variant_ids.qty_in_rent')
            )
    ...
    def _compute_display_name(self):
        ...
        for template in self:
            if template.rent_ok:
                template.display_name = self.env._(
                    "%s (Rental)", template.display_name,
                )

    def _get_best_pricing_rule(
        self, product=False, start_date=False, end_date=False, **kwargs
    ):
        """Return the best pricing rule for the given duration.

        :param ProductProduct product: a product recordset (at most one record)
    ...
    def _get_additional_configurator_data(
        ...
        **kwargs,
        ...
        :param datetime|None start_date: The rental start date.
        :param datetime|None end_date: The rental end date.
