...
class SaleOrderLine(models.Model):
    ...
    def _create_deposit_credit_note(self, qty_returned, qty_delivered):
        ...
        for deposit_line in deposit_lines:
            ...
            # Held deposits (active / released / forfeited) captured no refundable
            # cash via a normal payment: unhold and forfeit resolve them directly,
            # so no credit note is issued on return. Only cash-paid deposits (with
            # no hold payment) are refunded — this also avoids the "nothing left to
            # pay" crash and a forfeit-then-return double refund.
            if deposit_invoice.deposit_hold_payment_ids.filtered(lambda p: p.is_deposit_hold):
                continue
