...
class ProductProduct(models.Model):
    ...
    def _get_qty_in_rent(self):
        """Compute quantity currently in rent from confirmed sale order lines.

        Note: we don't use product.with_context(location=...).qty_available
    ...
    def action_view_rentals(self):
        """Access Schedule view of rental order lines, filtered on the current
        variants."""
        action = self.env['ir.actions.actions']._for_xml_id(
            'ggg_rental.action_rental_order_schedule',
        )
