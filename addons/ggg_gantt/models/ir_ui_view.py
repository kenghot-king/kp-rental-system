...
class IrUiView(models.Model):
    ...
    def _validate_tag_gantt(self, node, name_manager, node_info):
            ...
            if child.tag == 'templates':
                if not templates_count:
                    ...
                    self._raise_view_error(
                        _('Gantt view can contain only one templates tag'),
                        child,
                    )
            elif child.tag != 'field':
                self._raise_view_error(
                    _('Gantt child can only be field or template, got %s', child.tag),
                    child,
                )

        default_scale = node.get('default_scale')
        if default_scale and default_scale not in (
            'day', 'week', 'week_2', 'month', 'month_3', 'year',
        ):
            self._raise_view_error(
        ...
        if default_range and default_range not in (
            'day', 'week', 'month', 'quarter', 'year',
        ):
            self._raise_view_error(
                _("Invalid default_range '%s' in gantt", default_range), node,
            )

        display_mode = node.get('display_mode')
        if display_mode and display_mode not in ('dense', 'sparse'):
            self._raise_view_error(
                _("Invalid display_mode '%s' in gantt", display_mode), node,
            )

        attrs = set(node.attrib)
        if 'date_start' not in attrs:
            self._raise_view_error(
                _("Gantt must have a 'date_start' attribute"), node,
            )

        if 'date_stop' not in attrs:
            self._raise_view_error(
                _("Gantt must have a 'date_stop' attribute"), node,
            )

        if 'dependency_field' in attrs and 'dependency_inverted_field' not in attrs:
            self._raise_view_error(
                _(
                    "Gantt must have a 'dependency_inverted_field' attribute "
                    "once the 'dependency_field' is specified"
                ),
                node,
            )
        ...
        if remaining:
            self._raise_view_error(
                _(
                    "Invalid attributes (%(invalid_attributes)s) in gantt view. "
