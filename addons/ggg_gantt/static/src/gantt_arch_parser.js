...
const SCALES = {
    ...
    month: {
        ...
        groupHeaderFormatter: (date, env) =>
            date.toFormat(env.isSmall ? "MMM yyyy" : "MMMM yyyy"),
    ...
    year: {
        ...
        colHeaderFormatter: (date, env) =>
            date.toFormat(env.isSmall ? "MMM" : "MMMM"),
...
/**
 * Format a date to a week-based string with locale settings.
 *
            ...
            switch (node.tagName) {
                ...
                case "templates": {
                    ...
                    if (body) {
                        ...
                        for (const child of popoverArchParams.bodyTemplate
                            .childNodes) {
                            if (child.nodeType === 8) {
                                continue;
                            }
...
function getInfoFromRootNode(rootNode) {
    ...
    const consolidationMax = attrs.consolidation_max
    ...
    const precisionAttrs = attrs.precision
        ? evaluateExpr(attrs.precision)
        : {};
    for (const scaleId in SCALES) {
        if (precisionAttrs[scaleId]) {
            const precision = precisionAttrs[scaleId].split(":");
            if (
                precision[1] &&
    ...
    return {
        ...
        formViewId: attrs.form_view_id
            ? parseInt(attrs.form_view_id, 10)
            : false,
        kanbanViewId: attrs.kanban_view_id
            ? evaluateExpr(attrs.kanban_view_id)
            : null,
        multiCreateView: attrs.multi_create_view || null,
        pagerLimit: attrs.groups_limit
            ? parseInt(attrs.groups_limit, 10)
            : null,
        pillDecorations,
        progressBarFields: attrs.progress_bar
            ? attrs.progress_bar.split(",")
            : null,
