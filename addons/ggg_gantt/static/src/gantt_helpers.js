...
function closest(target, values) {
    return values.reduce(
        (prev, val) =>
            Math.abs(val - target) < Math.abs(prev - target) ? val : prev,
        Infinity
    );
}

/**
 * Add a time diff to a date keeping the same value even if the offset changed
 * during the manipulation (DST transitions).
 *
 * @param {DateTime} date
...
export function getCellColor(cellPart, subSlotUnavailabilities, isToday) {
    ...
    switch (cellPart) {
        ...
        case 2: {
            const [c0, c1] = subSlotUnavailabilities.map((d) =>
...
/**
 * Compute intersection of a closed interval with a union of ordered, disjoint intervals.
 *
 * @param {[DateTime, DateTime]} interval
...
export function getHoveredCellPart(cell, pointerX, cellPart, rtl) {
    ...
    let part = Math.floor(
        (clamp(pointerX, x, x + width - 1) - x) / (width / cellPart)
    );
    ...
    const findSiblings = (el) =>
        ref.el.querySelectorAll(
            related
                .map(
                    (attr) =>
                        `[${attr}='${el.getAttribute(attr).replace(/'/g, "\\'")}']`
                )
                .join("")
        );
...
function getBadgesPositions(el, rtl) {
    ...
    const sideBarWidth =
        document.querySelector(".o_gantt_row_sidebar")?.offsetWidth || 0;
    const startPosition = {
...
export const useGanttConnectorDraggable = makeDraggableHook({
    ...
    onDragStart: ({ ctx, addStyle }) => {
        ...
        for (const otherParent of ctx.ref.el.querySelectorAll(
            ctx.parentWrapper
        )) {
...
export const useGanttDraggable = makeDraggableHook({
    ...
    onDrag({ ctx, addStyle }) {
        const { cellSelector, current, getBadgesInitialDates, hoveredCell, scale } =
            ctx;
        let { el: cell, part } = hoveredCell;
        ...
        if (cell) {
            ...
            if (isDifferentCell || isDifferentPart) {
                ...
                const [gridRowStart, gridRowEnd] = /r(\d+) \/ r(\d+)/g
                    .exec(gridRow)
                    .slice(1);
    ...
    onWillStartDrag({ ctx, addCleanup, addClass }) {
        ...
        const gridTemplateColumns =
            gridStyle.getPropertyValue("grid-template-columns");
...
export const useGanttResizable = makeDraggableHook({
    ...
    onComputeParams({ ctx, params, addCleanup, addEffectCleanup, getRect }) {
        const onElementPointerEnter = (ev) => {
            ...
            const showHandles = params.showHandles
                ? params.showHandles(pill)
                : {};
    ...
    onDrag({ ctx, addStyle, getRect }) {
        ...
        if (current.isStart) {
            ...
            addStyle(current.pill, {
                "grid-column-start": `c${current.firstCol + diff}`,
            });
        } else {
            diff = Math.max(diff, 1 - current.initialDiff);
            addStyle(current.pill, {
                "grid-column-end": `c${current.lastCol + diff}`,
            });
        }
        ...
        const startDate = current.isStart
            ? dateAddFixedOffset(start, { [time]: diff * cellTime })
            : start;
        const stopDate = current.isStart
            ? stop
            : dateAddFixedOffset(stop, { [time]: diff * cellTime });
        return getBadges(
            current.pill,
            [startDate, startDiff],
            [stopDate, stopDiff],
    ...
    onWillStartDrag({ ctx, addClass }) {
        ...
        current.isStart =
            current.element.classList.contains(HANDLE_CLASS_START);
...
function getBlockBounds(current) {
    const startCol = Math.min(
        current.initialCellBounds.startCol,
        current.cellBounds.startCol
    );
    const endCol = Math.max(
        current.initialCellBounds.endCol,
        current.cellBounds.endCol
    );
    const startRow = current.rowId
        ? current.initialCellBounds.startRow
        : Math.min(
              current.initialCellBounds.startRow,
              current.cellBounds.startRow
          );
    const endRow = current.rowId
        ? current.initialCellBounds.endRow
        : Math.max(
              current.initialCellBounds.endRow,
              current.cellBounds.endRow
          );
    return { startCol, endCol, startRow, endRow };
}
...
/**
 * Same as usePopover, but replaces the popover with a dialog on small screens.
 *
 * @param {string} dialogTitle
 * @param {typeof import("@odoo/owl").Component} component
 * @param {Object} options
 */
export function useGanttResponsivePopover(
    dialogTitle,
    component,
    options = {}
) {
    ...
    const dialogAddFn = (_, comp, props, opts) =>
        dialogService.add(comp, props, opts);
    const popoverInDialog = makePopover(dialogAddFn, GanttPopoverInDialog, {
        onClose,
    });
    const ganttResponsivePopover = {
        open: (target, props) => {
            if (env.isSmall) {
    ...
    onWillUnmount(ganttResponsivePopover.close);
    return ganttResponsivePopover;
}
